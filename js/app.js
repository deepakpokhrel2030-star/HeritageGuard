  /* HeritageGuard app.js — complete with Azure Logic App + Blob Storage + Computer Vision */
  ...
  async function applyFilters(){
    ...
    /* Use Azure AI Search when there's a text query */
    if(q&&q.length>1){
      const searchResults=await searchWithAzureAI(q)
      if(searchResults&&searchResults.length>0){
        /* Map search results back to full ASSETS objects */
        const ids=new Set(searchResults.map(r=>r.id))
        res=ASSETS.filter(a=>ids.has(a.id))
        /* If no matches in local ASSETS, use search results directly */
        if(res.length===0)res=searchResults
        if(searchResults&&searchResults.length>0){
    const ids=new Set(searchResults.map(r=>r.id))
    res=ASSETS.filter(a=>ids.has(a.id))
    if(res.length===0)res=searchResults
    /* Show Azure AI Search banner */
    ...
    banner.style.cssText='background:linear-gradient(135deg,#0078d4,#005a9e);color:#fff;padding:.6rem 1rem;border-radius:8px;margin-bottom:1rem;font-size:.85rem;display:flex;align-items:center;gap:.5rem'
    ...
    res=ASSETS.filter(a=>
      ...
      (a.aiTags||[]).some(t=>t.toLowerCase().includes(q.toLowerCase()))
        ...
        /* Azure Search returned no results — fall back to local */
        res=ASSETS.filter(a=>
          (a.title||'').toLowerCase().includes(q.toLowerCase())||
          (a.description||'').toLowerCase().includes(q.toLowerCase())||
          (a.location||'').toLowerCase().includes(q.toLowerCase())||
  ...
  /* ============================================================
    ...
    Calls Azure CV API v3.2 to analyse image and return AI tags
    ============================================================ */
  ...
  async function doUpload(){
    ...
    /* STEP 1 — Upload file to Azure Blob Storage with progress tracking */
    let blobUrl=''
    try{
    ...
    /* STEP 2 — Video: generate thumbnail | Image: run Computer Vision */
    ...
    const newAsset={
      ...
      aiTags,          /* ← includes Azure Computer Vision tags for images */
      specs,
