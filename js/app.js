...
async function initDemoPasswords(){
  for(const u of USERS){
    if(u.pw&&u.pw.length<64){
      u.pw=await hashPassword(u.pw)
    }
  }
}
...
function delFromCard(id,title){const a=ASSETS.find(x=>x.id===id);const region=a?.region||'Asia';openModal('Delete this asset?',`"${title}" will be permanently removed from Azure.`,'Delete',()=>execDel(id,region))}
async function execDel(id,region){
  showLoad()
  try{
    if(USE_LIVE){await deleteAsset(id,region);ASSETS=await getAllAssets()}
    else{const idx=ASSETS.findIndex(a=>a.id===id);if(idx!==-1)ASSETS.splice(idx,1)}
    if(curAsset&&curAsset.id===id)curAsset=null
    hideLoad();toast('Asset deleted from Cosmos DB ✓','success');goPage('archive')
  }catch(e){
    hideLoad();console.error('Delete failed:',e)
    const idx=ASSETS.findIndex(a=>a.id===id);if(idx!==-1)ASSETS.splice(idx,1)
    if(curAsset&&curAsset.id===id)curAsset=null
    toast('Deleted locally (API error: '+e.message+')','warn');goPage('archive')
  }
}
...
async function delUser(uid,name){
  ...
  openModal('Remove user?',`"${name}" will be permanently removed from Cosmos DB.`,'Remove',async()=>{
    ...
    try{
      ...
      const idx=USERS.findIndex(u=>u.id===uid);if(idx!==-1)USERS.splice(idx,1)
      hideLoad();toast('User removed from Cosmos DB ✓','success');renderAdmin()
    }catch(e){
      hideLoad();console.error('Delete user error:',e)
      const idx=USERS.findIndex(u=>u.id===uid);if(idx!==-1)USERS.splice(idx,1)
      toast('User removed locally (API error)','warn');renderAdmin()
    }
  })
}
...
async function saveProfile(){
  ...
  try{
    ...
    hideLoad();toast('Profile saved to Cosmos DB ✓','success');renderProfile()
  }catch(e){
    hideLoad();console.error('Profile update error:',e)
    const idx=USERS.findIndex(u=>u.id===me.id)
    if(idx!==-1){USERS[idx]={...USERS[idx],first:fn,last:ln,email:em,org:org||'Public'};me=USERS[idx]}
    document.getElementById('chip-name').textContent=me.first+' '+me.last
    toast('Profile updated locally (API error)','warn');renderProfile()
  }
}
...
async function changePass(){
  ...
  const hashedCp=await hashPassword(cp)
  if(hashedCp!==me.pw){toast('Current password is incorrect.','error');return}
  ...
  showLoad()
  try{
    const hashedNp=await hashPassword(np)
    const updated={id:me.id,firstName:me.first,lastName:me.last,email:me.email,password:hashedNp,role:me.role,org:me.org||'Public',joined:me.joined||''}
    await fetch(CONFIG.ENDPOINTS.updateUser,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(updated)})
    const idx=USERS.findIndex(u=>u.id===me.id)
    if(idx!==-1){USERS[idx].pw=hashedNp;me=USERS[idx]}
    ;['p-cp','p-np','p-np2'].forEach(id=>{document.getElementById(id).value=''})
    hideLoad();toast('Password updated in Cosmos DB ✓','success')
  }catch(e){
    hideLoad();console.error('Password update error:',e)
    const hashedNp=await hashPassword(np)
    const idx=USERS.findIndex(u=>u.id===me.id)
    if(idx!==-1){USERS[idx].pw=hashedNp;me=USERS[idx]}
    ;['p-cp','p-np','p-np2'].forEach(id=>{document.getElementById(id).value=''})
